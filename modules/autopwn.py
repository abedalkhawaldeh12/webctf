                ...
                for val in candidates:
                    ...
                    try:
                        ...
                        if self._check_and_store_flags(r.text, f"Cookie brute-force ({cname}={val}) against {ep}"):
                            ...
                            found_flag = True
                            break
                        # Also check for actual flag patterns in GET response body
                        # (only match real flag formats, not generic words like 'flag' or 'cookie')
                        if re.search(r'(picoCTF\{[^}]+\}|flag\{[^}]+\}|CTF\{[^}]+\}|FLAG\{[^}]+\})', r.text, re.IGNORECASE):
                            print_success(f"  [Cookie BF] Flag pattern found for '{cname}'='{val}' on {ep} (GET)!")
                            self._log_step("Phase 4: Cookie BF", f"Flag pattern for {cname}={val}", details=ep)
                        ...
                        if is_post_endpoint:
                            ...
                            if self._check_and_store_flags(r.text, f"Cookie brute-force POST ({cname}={val}) against {ep}"):
                                ...
                                found_flag = True
                                break
                            # Also check for actual flag patterns in POST response body
                            # (some challenges reveal the flag in a different format or page)
                            if re.search(r'(picoCTF\{[^}]+\}|flag\{[^}]+\}|CTF\{[^}]+\}|FLAG\{[^}]+\})', r.text, re.IGNORECASE):
                                print_success(f"  [Cookie BF] Flag pattern found for '{cname}'='{val}' on {ep} (POST)!")
                                self._log_step("Phase 4: Cookie BF", f"Flag pattern for {cname}={val}", details=ep)
                                self._check_and_store_flags(r.text, f"Cookie BF success ({cname}={val})")
                                found_flag = True
                                break
